...
const bodyParser = require("body-parser");           //social buttons for bootsrap google or fb logo kinda
...
app.use(session({
  secret:"Damon Salvatore for life.",
  resave:false,
...
app.get("/secrets",function(req,res){
  User.find({secret:{$ne:null}}).then(function(data){
    if(data){
      res.render("secrets",{usersWithSecret:data});
    }
...
app.post("/submit",function(req,res){
  ...
  User.findById(req.user.id).then(function(data){
    if(data){
      data.secret=submittedRequest;
      data.save().then(function(){res.redirect("/secrets");})
    }
  }).catch(function(err){
